...
public class PlayerMovementPacket : Packet {
    ...
    public PlayerMovementPacket(Vector2 position, Guid? id = null) {
        this.ID = id;
    ...
    public override byte[] ToBytes() {
        ...
        var packetSize = sizeof(byte) + sizeof(float) + sizeof(float) + idBytes.Length;
        ...
        Array.Copy(idBytes, 0, packetBytes, sizeof(float) + sizeof(float) + 1, idBytes.Length);
    ...
    public new static PlayerMovementPacket FromBytes(byte[] packetBytes) {
        ...
        var id = new byte[packetBytes.Length - (sizeof(byte) + sizeof(float) + sizeof(float))];
        Array.Copy(packetBytes, sizeof(byte) + sizeof(float) + sizeof(float), id, 0, id.Length);

        return new PlayerMovementPacket(new Vector2(positionX, positionY), id.Length > 0 ? new Guid(id) : null);
    }
